...
iot_grower::iot_grower()
{
  ...
  // 
  const unsigned long timer_duration_1 = 10; // Duration for the first timer in minutes
  const unsigned long timer_duration_2 = 20;
  const unsigned long timer_overflow_count_1 = (timer_duration_1 * 60 * F_CPU) / 1024; // Calculate the overflow count for the first timer
  const unsigned long timer_overflow_count_2 = (timer_duration_2 * 60 * F_CPU) / 1024;
  ...
  OCR1A = timer_overflow_count_1; // Set the Output Compare Register A to the overflow count
...
iot_grower::timer1_ISR(TIMER1_COMPA_vect)
{
...
iot_grower::timer2_ISR(TIMER2_COMPA_vect)
{
  iot::grower::readDHT();
  iot::grower::readWaterLevel();
  iot_grower::updateCloud();
}
...
void iot_grower::update_Cloud()
{
  ArduinoCloud.update();
}

void iot_grower::activate_lamp()
{
  PORTD &= ~(1 << 6);
}

void iot_grower::deactivate_lamp()
{
  PORTD |= (1 << 6);
}
